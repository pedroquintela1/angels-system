...
export function usePermissions() {
  ...
  /**
   * Check if user is admin (ADMIN only)
   */
  ...
  /**
   * Check if user is super admin (same as admin now)
   */
  ...
  const hasSupportAccess = (): boolean => {
    return hasAnyRole([UserRole.SUPPORT, UserRole.ADMIN]);
  };
  ...
  const hasFinancialAccess = (): boolean => {
    return hasAnyRole([UserRole.FINANCIAL, UserRole.ADMIN]);
  };
